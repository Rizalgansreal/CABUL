...
def banner():
    clear()
    lo = f'{P2}[{H2}+{P2}] Author       : {H2}RizalCyber\n{P2}[{H2}+{P2}] Instagram    : {H2}RizalCyber\n{P2}[{H2}+{P2}] Version      : {H2}2.0'
    cetak(Panel(lo,style='#FFFFFF'))
...
def login_kamu():
    if "sukses" in lisensiku:
        try:
            ...
            banner()
            lo = f'{P2}[{H2}+{P2}] Harap login terlebih dahulu\n\n{P2}[1] Login menggunakan {H2}cookie instagram\n{P2}[2] Login menggunakan {H2}username {P2}& {H2}password'
            cetak(Panel(lo,style='#FFFFFF'))
            loginpil=input(f"  {P}[{H}?{P}] Pilih {H}1 {P}sampai {H}2 {P}:{H} ")
            if loginpil=='1':
